...
const Menu = () => {
  ...
  return (
    <motion.div
      variants={{
        hidden: { translateX: "512px" },
        visible: { translateX: "0px" },
      }}
      initial={variant}
      animate={variant}
      transition={{ duration: 0.2 }}
      className="z-10 container-start-v gap-7 text-start px-16 overflow-y-auto h-screen w-128 bg-primary fixed top-0 right-0"
    >
      <span className="text-dark text-6xl select-none">ELSAKKA</span>
      <p className={`${space_grotesk.className} text-dark grote select-none `}>
        Achieve your fitness goals with personalized training. We create custom
        workout plans for weight loss, muscle building, and overall health.
